...
module {
  ...
  public let OPENAI_API_KEY : Text = "sk-PLACEHOLDER_OPENAI_KEY_REPLACE_ME";
  ...
  // ── Chat AI notification helpers ─────────────────────────────────────────────

  /// Send the AI reply to the user's own email address (branded template).
  public func sendChatReplyToUser(
    userName : Text,
    userEmail : Text,
    question : Text,
    aiReply : Text,
    ...
    let subject = "Your Vidyamandir AI Assistant Response";
    let bodyHtml =
      "<p>Dear " # escapeJson(userName) # ",</p>" #
      "<p>Thank you for reaching out to Vidyamandir! Here is the response to your question:</p>" #
      "<p><strong>Your Question:</strong></p>" #
      "<blockquote style=\"border-left:4px solid #e65c00;padding:8px 16px;background:#fff8f5;margin:8px 0\">" #
      escapeJson(question) #
      "</blockquote>" #
      "<p><strong>AI Assistant Reply:</strong></p>" #
      "<blockquote style=\"border-left:4px solid #0a1628;padding:8px 16px;background:#f0f4ff;margin:8px 0\">" #
      escapeJson(aiReply) #
      "</blockquote>" #
      "<p style=\"color:#888;font-size:13px\">This response was generated by our AI assistant. " #
      "For further help, visit our store or contact us directly.</p>";
    // Send to user's email (override the default STORE_EMAIL target)
    let url = "https://api.resend.com/emails";
    let payload =
      "{" #
      "\"from\":\"" # escapeJson(FROM_EMAIL) # "\"," #
      "\"to\":[\"" # escapeJson(userEmail) # "\"]," #
      jsonStr("subject", subject) # "," #
      jsonStr("html", htmlWrap("AI Assistant Response", bodyHtml)) #
      "}";
    let headers : [OutCall.Header] = [
      { name = "Content-Type"; value = "application/json" },
      { name = "Authorization"; value = "Bearer " # RESEND_API_KEY },
  ...
  /// Send notification to the store about a new chat enquiry (including full conversation).
  public func sendChatEnquiryToStore(
    enquiryId : Text,
    userName : Text,
    userEmail : Text,
    userPhone : Text,
    ...
    let subject = "New Chat Enquiry from " # userName # " \u{2014} Vidyamandir";
    let bodyHtml =
      "<p>A new AI chat enquiry has been received on Vidyamandir.</p>" #
      table(
        row("Enquiry ID", enquiryId) #
        row("Name", userName) #
        row("Email", userEmail) #
        row("Phone", if (userPhone == "") "Not provided" else userPhone)
      ) #
      ...
      escapeJson(aiReply) #
      "</blockquote>";
    await sendEmail(subject, htmlWrap("New Chat Enquiry", bodyHtml), transform);
  };

  /// Send an SMS to the user's own phone number with the AI reply summary.
  public func sendChatSmsToUser(
    userPhone : Text,
    aiReply : Text,
    transform : OutCall.Transform,
  ) : async () {
    // Truncate aiReply to 120 chars for SMS
    let snippet = if (aiReply.size() > 120) {
      Text.fromArray(aiReply.toArray().sliceToArray(0, 120))
    } else {
      aiReply
    };
    let smsBody = "Vidyamandir AI: " # snippet # " (Full reply sent to your email)";
    // Build E.164 phone from user-supplied number
    let e164Phone = toE164(userPhone);
    let url = "https://api.twilio.com/2010-04-01/Accounts/" # TWILIO_ACCOUNT_SID # "/Messages.json";
    let credentials = base64Encode(TWILIO_ACCOUNT_SID # ":" # TWILIO_AUTH_TOKEN);
    let formBody =
      "To=" # urlEncode(e164Phone) #
      "&From=" # urlEncode(TWILIO_FROM_NUMBER) #
      "&Body=" # urlEncode(smsBody);
    let headers : [OutCall.Header] = [
      { name = "Content-Type"; value = "application/x-www-form-urlencoded" },
      { name = "Authorization"; value = "Basic " # credentials },
    ];
  ...
  /// Convert a 10-digit Indian phone to E.164 (+91XXXXXXXXXX).
  func toE164(phone : Text) : Text {
    var digits = "";
    ...
    if (digits.size() == 10) {
      "+91" # digits
    } else if (digits.size() == 12 and Text.fromArray(digits.toArray().sliceToArray(0, 2)) == "91") {
      "+" # digits
    } else {
      "+" # digits
    };
